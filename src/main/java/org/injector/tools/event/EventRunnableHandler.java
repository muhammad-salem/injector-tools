...
public interface EventRunnableHandler {
    ...
    default void addCompleteListener(Runnable listener) {
        completeListenerList.add(listener);
    }

    default void addErrorListener(Runnable listener) {
        errorListenerList.add(listener);
    }

    default void addStartListener(Runnable listener) {
    ...
    default void addSuccessListener(Runnable listener) {
        successListenerList.add(listener);
    }

    default void addInitListener(Runnable listener) {
        initListenerList.add(listener);
    }


    default void fireCompleteListener() {
        completeListenerList.forEach(Runnable::run);
    }

    default void fireInitListener() {
        initListenerList.forEach(Runnable::run);
    }

    default void fireErrorListener() {
        errorListenerList.forEach(Runnable::run);
    }

    default void fireStartListener() {
        startListenerList.forEach(Runnable::run);
    }

    default void fireStopListener() {
        stopListenerList.forEach(Runnable::run);
    }

    default void fireSuccessListener() {
        successListenerList.forEach(Runnable::run);
    }
    ...
    default boolean removeCompleteListener(Runnable listener) {
        return completeListenerList.remove(listener);
    }

    default boolean removeInitListener(Runnable listener) {
        return initListenerList.remove(listener);
    }

    default boolean removeErrorListener(Runnable listener) {
        return errorListenerList.remove(listener);
    }

    default boolean removeStartListener(Runnable listener) {
        return startListenerList.remove(listener);
    }

    default boolean removeStopListener(Runnable listener) {
        return stopListenerList.remove(listener);
    }

    default boolean removeSuccessListener(Runnable listener) {
